...
const Chronometer = (props) => {
    ...
    const handleStartCount = ()=> {
        ...
        if(!intervalId){
		 setInterval(()=>{
			(minute == 0) ? ((second == 0) ? pauseTime() : second-- ) 
							: ((second == 0) ? (minute--, second='59') : second--)
            
            
            setTime({
                minute: (minute < 10 && typeof minute === 'number' )? "0" + minute : minute,
                second: (second < 10 && typeof minute === 'number' )? "0" + second : second
            })
    ...
    return (
        <div className="pomodoro-container">
            ...
            <div className="pomodoro-buttons">
                <button id="start-button" onClick={handleStartCount}>Começar</button>
